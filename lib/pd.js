...
/**
 * Каталог source-capabilities партнёра. Layout зависит от ядра:
 *   новый:   <partner>/pdN/source-capabilities            (N варьируется)
 *   старый:  <partner>/usb_power_delivery/source-capabilities
 * null если партнёра/PD нет.
 */
...
/**
 * PDO-профили зарядника из source-capabilities партнёра. [] если нет.
 * Значения в sysfs с суффиксами (5000mV / 3000mA) — parseInt их срезает.
...
export async function readPartnerPdos(port) {
    ...
    for (const e of entries) {
        ...
        if (type === 'fixed_supply') {
            ...
            pdos.push({
                type, watts: null,
                vmin: mv2v(vmin), vmax: mv2v(vmax), // числовой диапазон для active-подсветки
...
/**
 * Индекс активного PDO для негоциированного вольтажа negV (В).
 * Сначала fixed по совпадению вольтажа, иначе PPS чей диапазон содержит negV.
 * -1 если ничего не подошло.
 */
