// USB device enumeration via GUdev. Sync (fast C calls + tiny sysfs attrs).
// The GUdev.Client is passed in (reused from UdevMonitor).

// bDeviceClass (hex) → human-readable. 00 = class defined at the interface level.
const CLASS_NAMES = {
    0x00: 'Per interface',
    0x01: 'Audio',
    0x02: 'Communications',
...
// bDeviceClass → symbolic icon (all present in Adwaita).
const CLASS_ICONS = {
...
/**
 * List of USB devices. Root hubs (usbN) are filtered out.
 * @param {GUdev.Client} client — the reused client (from UdevMonitor).
 */
export function listUsbDevices(client) {
    ...
    for (const d of devices) {
        ...
        // removable: 'removable' (external port) | 'fixed' (built-in) | 'unknown'
        const removable = attr('removable') || 'unknown';
        const maxPower = attr('bMaxPower') || ''; // requested current, e.g. "500mA"
    ...
    // Stable order by port path.
    out.sort((a, b) => a.name.localeCompare(b.name, undefined, {numeric: true}));
    return out;
}

/**
 * Whether a device is ignored for hide-when-idle.
 * ignoreList: strings "vid:pid" or "class:XX" (hex).
 */
